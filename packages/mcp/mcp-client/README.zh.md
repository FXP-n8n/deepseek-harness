...
| `protocolEra` | `legacy` | 使用的协议版本：`legacy`（仅 2025 握手）、`auto`（服务器支持时使用 2026-07-28，否则回退）或 `{ pin: '2026-07-28' }`（强制要求该版本） |
| `maxRounds` | `10` | 一次 `tools/call` 在失败前可用于应答服务器输入请求的轮数 |
...
在 2026-07-28 版本中，服务器可以暂停一次工具调用向人提问——一次确认、一个选择或一张简短表单。harness 通过其常规提问界面呈现，并将答案回传，服务器随后继续同一次调用。只要提问服务已挂载即可工作，标准 harness 配置均已挂载该服务。

桥接只应答服务器实际提出的问题。它绝不声明 URL 形式的 elicitation——那需要一个没有任何 harness 客户端实现的同意与导航界面——也绝不声明已弃用的 sampling、roots 或 logging 能力：请求这些能力的服务器会得到明确错误，而不是被静默降级的回答。跳过或取消该提问会取消服务器的请求，而不是给出答案；值为空的提交会报告为 decline，因此模型绝不会看到凭空编造的答案。

### 启动、工具更新与重连

服务器的工具会在 harness 开始首个轮次之前出现。服务器更改工具列表时，模型的工具集会自动更新；更新失败时，上一组工具继续可用。该变更通过服务器声明相应能力后客户端开启的列表变更订阅到达，因此未声明该能力却更改列表的服务器会在下次重连或重载时才被发现。
...
- **协议版本是配置，不是升级的副作用。** 除非配置项明确选择，连接都使用 2025 握手，因此升级桥接不会改变既有服务器在线路上看到的内容。
...
| [`src/cache.ts`](src/cache.ts) | `tools/list` 描述符缓存，按服务器 namespace 与授权划分 |
| [`src/elicitation.ts`](src/elicitation.ts) | 表单模式 elicitation：问题映射、agent 归属、请求处理器 |
...
`apply` 解析重连策略与协议版本、在当前注册作用域内预留 `serverName`、启动监督器，并等待初始连接加发现完成。独立 agent（智能体）作用域可以复用相同 namespace，因为其工具与传输彼此隔离；同一作用域内重复会在加载时失败。声明能力来自实际挂载的 seam——仅提问服务，且仅表单模式——当该 seam 存在时，桥接会在连接前注册其 elicitation 处理器。监督器把所有同步——初始、列表变更与重连——串行到同一条队列，因此两次同步绝不会交错执行各自的先 dispose 后注册交换。dispose 会取消待执行的重连、关闭活动客户端、等待进行中的尝试与排队同步完全停稳，然后注销当前世代。

2026-07-28 版本的 `tools/list` 结果带有新鲜度窗口与共享范围。桥接按 `(serverName, 授权摘要)` 存储聚合后的描述符——从不是绑定到客户端的定义——并在该窗口内提供服务，因此重连或同级的 agent 作用域会复用同一次发现，而不必再次排空分页；列表变更或 dispose 会清除该条目。摘要始终是键的一部分，因此服务器的 `public` 范围只会被当作“可缓存”，绝不会扩大为跨授权复用。

当服务器声明该能力并宣告变更时，SDK 会调用桥接的列表变更处理器；在 2026-07-28 版本中，该通知通过客户端开启的订阅到达。处理器清除缓存列表并排队一次重新同步；获取阶段失败时保留上一世代注册，注册冲突则回滚本次尝试的世代。每次中断共享一个尝试预算：连续失败达到 `maxAttempts` 次后工具被注销、重连停止；连接存活超过 `maxDelayMs` 会重置预算。
...
- **2026-07-28 版本需显式启用且只被部分消费**——除非 `protocolEra` 选择现代修订版，连接都使用 `initialize` 握手；此外没有任何 harness 客户端实现 URL 形式的 elicitation 或已弃用的 sampling、roots、logging 能力，因此请求这些能力的服务器会明确失败。参见[迁移说明](../../../.agents/notes/implemented/feature/2026-09-12-mcp-2026-07-28-client-migration.zh.md)。
- **来自不同 agent 的工具调用共用每个服务器的一条提问通道**——服务器发起的提问会归属到该连接上唯一有调用在飞的 agent；当不同 agent 的调用同时在飞时，桥接会取消该提问，而不是把它展示给错误的 agent。
